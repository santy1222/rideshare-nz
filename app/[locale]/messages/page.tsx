...
export default async function MessagesPage() {
  ...
  for (const msg of (messages as Message[]) ?? []) {
    ...
    if (!convMap.has(key)) {
      convMap.set(key, {
        tripId: msg.trip_id,
        otherId,
        otherName: (isSender ? msg.receiver?.full_name : msg.sender?.full_name) ?? t("user"),
  ...
  const dateLocale = locale === "en" ? "en-NZ" : "es";

  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <h1 className="font-display font-semibold text-2xl text-gray-900 mb-6">{t("title")}</h1>

      {conversations.length === 0 ? (
        <div className="card text-center py-14">
          <MessageCircle size={32} className="text-gray-300 mx-auto mb-3" />
          <p className="font-medium text-gray-500 text-sm">{t("empty")}</p>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      ...
                      <span className="text-xs text-gray-400 flex-shrink-0">
                        {new Date(conv.lastDate).toLocaleDateString(dateLocale, { day: "numeric", month: "short" })}
                      </span>
                    </div>
