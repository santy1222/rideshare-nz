...
import { MapPin, Calendar, Clock, Users, DollarSign, Phone, Star, Lock, ArrowLeft } from "lucide-react";
...
export default async function TripDetailPage({ params }: PageProps) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <Link href="/" className="flex items-center gap-2 text-gray-500 hover:text-brand-700 mb-6 font-medium transition-colors">
        <ArrowLeft size={18} />
        {t("backToTrips")}
      </Link>
      ...
      <div className="card mb-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              <div className="flex items-center gap-2 bg-brand-50 px-3 py-2 rounded-xl min-w-0">
                ...
                <span className="font-display font-semibold text-brand-900 text-base sm:text-lg truncate">{trip.origin}</span>
              </div>
              <span className="text-xl text-gray-400">→</span>
              <div className="flex items-center gap-2 bg-brand-50 px-3 py-2 rounded-xl min-w-0">
                <MapPin size={15} className="text-brand-500 shrink-0" />
                <span className="font-display font-semibold text-brand-900 text-base sm:text-lg truncate">{trip.destination}</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 text-sm text-gray-600">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {t("seatsAvailable", { count: trip.seats_available })}
              </span>
            </div>
          </div>
          ...
          <div className="sm:text-right shrink-0 flex sm:flex-col items-center sm:items-end gap-2 sm:gap-0">
            {trip.price != null ? (
              ...
              <div className="text-gray-400 italic text-lg">{t("toArrange")}</div>
            )}
        ...
        <div className="border-t border-gray-100 pt-5">
          <h3 className="font-display font-semibold text-gray-800 mb-4">{t("driver")}</h3>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div>
                ...
                {driver?.avg_rating > 0 && (
                  <div className="flex items-center gap-1 mt-0.5">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star key={s} size={13} className={s <= Math.round(driver.avg_rating) ? "text-amber-400 fill-amber-400" : "text-gray-200 fill-gray-200"} />
                    ))}
                    <span className="text-xs text-gray-500 ml-1">({driver.total_reviews})</span>
                  </div>
                )}
              </div>
            </div>
            ...
            {user ? (
              driver?.phone ? (
                <a href={`tel:${driver.phone}`} className="flex items-center justify-center gap-2 bg-brand-500 hover:bg-brand-700 text-white px-4 py-2.5 rounded-xl font-semibold text-sm transition-colors min-h-[44px]">
                  <Phone size={15} />
                  {driver.phone}
                </a>
              ) : (
                <span className="text-gray-400 text-sm italic">{t("noPhone")}</span>
              )
            ) : (
              <Link href={`/login?redirect=/trips/${trip.id}`} className="flex items-center justify-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-600 px-4 py-2.5 rounded-xl font-medium text-sm transition-colors min-h-[44px]">
                <Lock size={14} />
                {t("signInToSeePhone")}
              </Link>
            )}
            ...
            {user ? (
              <>
                ...
                <ContactForm
                  ...
                  receiverName={driver?.full_name ?? ""}
                  hasConversation={!!existingMessage}
                />
              </>
            ) : (
              <Link href={`/login?redirect=/trips/${trip.id}`} className="w-full py-3 bg-brand-500 hover:bg-brand-700 text-white font-semibold rounded-xl transition-colors flex items-center justify-center gap-2 text-sm">
                {t("signInToJoin")}
              </Link>
            )}
          </div>
        ...
        {passengers && passengers.length > 0 && (
          <div className="border-t border-gray-100 pt-5 mt-5">
            <h3 className="font-display font-semibold text-gray-800 mb-3">
              {t("confirmedPassengers")} ({passengers.length})
            </h3>
            {user ? (
        ...
        {isOwner && (
          <div className="border-t border-gray-100 pt-4 mt-4">
            <CancelTripButton tripId={trip.id} label={t("cancelTrip")} />
          </div>
        )}
      </div>

      <div className="card">
        <h3 className="font-display font-semibold text-gray-800 mb-5">{t("driverReviews")}</h3>
            ...
            {(reviews as (Review & { reviewer: { full_name: string } })[]).map((review) => (
              <div key={review.id} className="border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-sm text-gray-700">{review.reviewer?.full_name}</span>
...
function CancelTripButton({ tripId, label }: { tripId: string; label: string }) {
  return (
